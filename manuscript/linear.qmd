...
But why do we need Shapley values for linear regression anyway?
Couldn't we just look at the coefficients?
...
So, surely, it isn't the best explanation for how a feature affected the prediction.
...
The Shapley value in the linear case is:
...
To better understand the feature effects, we can compute Shapley values for one of the data instances.
...
And to finally compute Shapley values we call the `.shap_values` method from the explainer.
...
- Most feature values of this wine were assigned a positive Shapley value.
- pH of 3.09 is the feature value with the largest negative Shapley value.
...
Shapley values allows a local interpretation -- how features affected a prediction.
...
## Comparing coefficients and Shapley values
