...
SHAP is based on Shapley values which are originally from game theory.
However, before diving into the theory and estimation of SHAP, let's first discuss the intuition of how Shapley values in general work.
...
When we dive into the origins of Shapley values, they were initially devised to address the issue of fair attribution in cooperative games.
...
## How to calculate the SHAP for one feature
...
The SHAP value is the average of all the marginal contributions to all possible coalitions.
...
![All 8 coalitions needed for computing the exact SHAp value of the `cat-banned` feature value.](images/shapley-coalitions.jpg)
...
In fact, there are various ways to estimate SHAP values as you can see in the [Estimation Chapter](#estimation).
...
These super-pixels are treated as a "feature" for SHAP values.
