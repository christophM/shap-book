...
The deep explainer is specifically for deep neural networks.
...
Connections to DeepLIFT

- DeepLIFT is a recursive explanation methods for feature attribution in (deep) neural networks
- Attributes to an input $x_j$ a contribution value
- Works with a reference data point instead of a background data set
- Binary representation of $x = h_x(\tilde{x})$ where 1 stands for input from origina data point to be explained and 0 means the value comes from reference data point.
...
- For each input feature, compute the contribution by comparing the activation of a neuron with that feature to the activation with the reference point.
- This comparison is 
$$ \Delta i(x) = \sum_{j} (a(x)_j - a(\eta)_j) * \frac{w_{ij}}{\sum_{k} w_{ik} $$
...
- Especially useful for strongly correlated features.
- 
...
TODO: Add an overview table of all explainers
columns: name of explainer, short descrption, targeted model, background data y/n,e
