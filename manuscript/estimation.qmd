...
It also utilizes all of the background data, not just a sample.
...
Because of this enumeration, the exact explainer can utilize "Gray code," an optimization method not applicable to other SHAP estimation methods.
...
As we are in a linear regression context, we can utilize standard tools for regression.
...
The Tree Explainer utilizes the tree structure to compute SHAP values, and it comes in two versions: Interventional and Tree-Path Dependent Explainers.
...
[^og-meaning]: OG refers to the original Explainer as proposed in the paper.
