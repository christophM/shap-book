...
The exact explainer computes all $2^(p-1)$ possible coalitions for each feature, so $2^p$ in total and sums over them as described in the [theory chapter](#theory).
...
The exact explainer is model-agnostic.
It's very expensive of course.
The complexity is $\mathcal{O}(2^p)$ .
...
Another cool thing though: You can also compute Owen's values.
This requires a clustered structured masker though.
TODO: move to own chapter

- enumerates all possible coaliations
- also implements grey code
- TODO: WRITE  about grey code
