...
This chapter present the various SHAP estimators in detail.
...
The first term denotes the effect of feature value $x_j^{(i)}$, while the second term centers it at the expected effect of feature $X_j$.
...
The algorithm must keep track of the cumulative weight of the subsets in each node, which complicated the algorithm.
...
shap.explainers.Tree(
  model, data, feature_pertubation='interventional'
)
shap.explainers.Tree(
  model, feature_pertubation='tree_path_dependent'
)
```
...
- Another interpretation of Deep Estimator: it computes the SHAP values in smaller parts of the network first and combines those to obtain SHAP values for the entire network, explaining the prediction from the input, similar to our usual understanding of SHAP.

::: {.callout-note}

How large should the background data be for Deep Estimator?
...
Partition Estimator also prove useful for image inputs, where image pixels can be grouped into larger regions.
