...
Although $X_2$ has a greater influence on the prediction due to its larger scale, the Shapley importance for both features is the same.
...
This demonstrates that interpreting interactions, which are inherently multiplicative, is not always intuitive.
...
Ideally, we would articulate and define the properties of the data and model that interest us and
...
As demonstrated in the chapters about [linear models](#linear) and [additive models](#additive), at least for these more restricted models, the results align with our expectations and what similar metrics (such as coefficients or PDP) would indicate.
...
## Shapley Value Method is not a Surrogate Model
...
## Possibility of Fooling Shap

It is **possible to create intentionally misleading interpretations** with SHAP, which can conceal biases[@slack2020fooling].
...
Refer to the [correlation chapter](#correlation) for more information.
