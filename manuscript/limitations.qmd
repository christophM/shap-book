...
## Computation Time can be quite a lot 

- Especially some "older" estimation version are quite slow, like Kernel SHAP and Sampling
- Model-specific versions are faster
- but still, all in all, Shapley values not the resource efficient method


## Not as interpretable as one would wish for

- I tried my best to convey it
- we have seen that for linear and additive models Shapley values mostly match what we would expect
- but interactions already more tricky
- and the shapley values don't have a simple interpretation
- For exmaple: Is feature value with twice the shapley value twice as important?
- not matching what humans would expect [@kumar2020problems]
