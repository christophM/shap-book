...
::: {.callout-note}
Maskers are the technical solution for "removing" feature values to compute marginal contributions and Shapley values.
:::

In this chapter, we explain the role of maskers.
To apply Shapley values from game theory to machine learning interpretability, we need to frame the prediction as a game, with feature values of a data point being the individuals.
The definition of feature values can be flexible, but we'll discuss that later.

Shapley values are computed by considering many possible contributions.
Forming a coalition of features means some features are excluded or absent.
...
Depending on the Explainer, this can involve sampling from the background data or iterating through all background data for each instance and feature value to compute the Shapley values.
...
The choice of background data influences the interpretation of Shapley values.

Some examples of what you can do by leveraging the background data:

- Simulate a distribution shift and obtain Shapley values under the shifted data.
- Analyze how Shapley values differ in different data groups.
...
The partition masker is used in combination with the [Partition explainer](#estimation) and includes a max_samples argument controlling the number of data points sampled from the background data to compute Shapley values.
...
However, when computing Shapley values, the internal representation is not our concern.
...
The next question is: How do we represent the "absence" of a word, or how do we mask a missing word?
This decision is up to the user.
By default, the word is replaced by an empty string, but it could also be replaced with "..." or even a randomly chosen word from background data or based on a grammar tool.
The chosen method will influence the prediction, as shown in the [text chapter](#text).
The Text masker in SHAP is used to implement masking.
...
_TODO: Read https://arxiv.org/pdf/2105.13787.pdf_

In the next chapter, we will explore correlations and alternatives for Shapley values.
