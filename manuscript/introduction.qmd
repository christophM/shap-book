...
However, there are many practical reasons why interpretability is important, beyond just explaining ML diagnoses to patients.
...
The main issue here is not interpretability but rather the inclusion of the right features.
...
This protocol involves a rapid response (RRT) nurse who monitors the alarms and informs the doctors, who then treat the patient.
...
Interestingly, a side-note in the report mentioned that the deep learning system provided no explanations for warnings, so it was unclear why a patient was likely to develop sepsis.
...
The prediction is a weighted sum (linear model) or based on intelligible rules.
...
Boosted trees often seem to be the best option in many cases.TODO: CITE
...
Boosted trees and random forests are already ensembles.
Feature transformations, like PCA, make features less interpretable.

So, what to do?

Even if you end up with an interpretable model, this book can help.
...
There are many methods that work with this principle:
