...
![In reality, asthma increases likelihood of pneumonia. However, in the study, asthma also increased the (preemptive) use of antibioitics which protects against pneumonia.](images/asthma-dag.jpg){#fig-asthma}
...
Junaid;
It may be worth expanding on this and including a motivating example. What is a payout, what constitutes a coalition. Usually the taxi sharing example is a good one from game theory.

Joshua:
Second this: I like the theoretical grounding of shap in cooperative game theory -- think some extra material here is warranted... even a simple diagram / illustration

Some nice examples from auction theory pricing or group project / assignment grades are in the economics literature -- could tie in to the theory section later (haven't got there yet)
...
Let's say you have a game with team players, they get some payout, and now the goal is to fairly share the payout among the players.
If the players contributed differently, it would be fair if this is reflected in the individual payouts.
Shapley values is a method to do this fairly.
An example could be a taxi ride.
While not a game, Shapley values still work here:
You share a taxi with 2 friends and you want to fairly attribute the payout.
Well, in this case, it's negative because you have to pay the taxi driver, but Shapley values work as well here.
The idea behind Shapley values is to compute how much each of the players contributed to the payout in various constellations to the team.
So you would calculate how much the taxi would have cost if only your two friends would have taken it, without you.
How much did the price change when adding you?

TODO: an image with the attributions for the taxi
TODO: Also add example with image of taxi ride where everyone goes
...
Let's talk now how this concept is useful to explain predictions.
...
We'll dive deeper into the distinction between SHAP and Shapley values in the  [history chapter](#history).
...
It's also the name of the library `shap` in Python, which we are going to use in this book.
