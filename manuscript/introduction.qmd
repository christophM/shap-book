...
That the model relied on the asthma feature in this way was only discovered because the model was interpretable.
...
Ensembles of models often produce the best esolutions.
...
To motivate SHAP, we first have to talk about how to fairly share a taxi.
Alice, Bob, and Charlie ride home from a dinner.
They share a taxi which costs \$45.
Alice and Bob are a couple and live together.
The taxi driver first drops them off, then drives further to drop Charlie off.
Splitting the bill by 3 wouldn't be completely fair.
How much did each of them contribute to the costs?

For that we can use a method called Shapley values.
This requires calculating for each combination of Alice, Bob, and Charlie how much the ride would have cost.
The next table shows this:

| Passengers           | Costs | Note |
|----------------------|-------| ---- |
| {}                   |   $0  | No taxi ride, no costs |
| {Alice}              |   $15 | Normal ride fare |
| {Bob}                |   $25 | Bob always insists of getting luxury taxis |
| {Charlie}            |   $38 | Charlies lives a bit further away |
| {Alice, Bob}         |   $25 | Bob always gets his way | 
| {Alice, Charlie}     |   $41 | Alice & Bob's place requires a slight detour | 
| {Bob, Charlie}       |   $51 | Bob requires luxury |
| {Alice, Bob, Charlie}|   $51 | Alice and Bob live together |

But still more work is required to find out how much each of them actually has to pay.
To do that, starting with Alice, we have to calculate how much costs she adds when she is added to every possible passenger constellation -- in this case to {Bob}, to {Charlie}, and to the empty set. 

Adding Alice to an empty taxi raises the costs from \$0 to \$15, adding her to a taxi with Bob already in it doesn't change the price of \$25, and the same is true if both Bob and Charlie are already in it (\$45).

We also do these calculations for Bob and Charlie and do some calculations (more in the [Theory Chapter](#theory).

As a result we get the individual costs:
...
## Shapley Values, a solution from game theory

A way to explain predictions comes from game theory and is called Shapley values[@shapley1953value], named after their inventor, Lloyd Shapley.
Shapley values are a method for fairly attributing the payout to each player in a coalition.
Shapley values were invented to solve the problem of fair attribution.
To return to the taxi example: Each passenger is a player, the coalition is the ride share, the payout in this case is the cost.
And the goal was to fairly share the costs between the passengers.
Shapley values is a method to do this "fairly".
Shapley values not only refer to the method but also to the resulting values, which can be confusing.
Shapley values are motivated by axioms of what a fair attribution is.
But more on that in the [Theory Chapter](#theory).
...
- Attributions: the resulting values from Shapley values / SHAP

:::


Let's talk now how this concept from game theory (Shapley values) is useful to explain predictions.

## SHAP is an explainable AI technique


SHAP is the application of Shapley values in Explainable AI [@NIPS2017_7062].
...
Since then, many other algorithms have been proposed to compute Shapley values for the attributing a prediction to the features.
...
- A prediction is like a game, and the payout is the prediction itself, minus the average prediction of all (test) data[^naive-model] .
