...
| Symbol | Description |
|--------|-------------|
| \hat{f}(x) | The prediction for the instance 'x' using the model 'f'. |
| \beta₀, \beta₁, ... , \betaₚ | The coefficients or weights of the linear model. They quantify the impact of each feature on the prediction.|
| x₁, ..., xₚ | These represent the feature values of the instance 'x'. Each feature is indexed by a subscript number.|
| xⱼ | This represents the 'j-th' feature value of the instance 'x'.|
| \phiⱼ(\hat{f}) | This represents the contribution of the 'j-th' feature to the prediction \hat{f}(x).|
