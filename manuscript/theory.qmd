...
So for now you had to believe me that for linear model the formula above indeed are related to Shapley values.
Let's actually see the definition of Shapley values for the general case.

## How Shapley values are defined

The Shapley values requires a value function $val$ which determines a payout and which takes a set of players $S$ (in our case feature values of a data point) as input.
...
$$val_{x}(S)=\int\hat{f}(x_{1},\ldots,x_{p})d\mathbb{P}_{X_{\bar{S}}}-E_X(\hat{f}(X))$$
