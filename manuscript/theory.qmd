...
Until now, you had to believe me that for a linear model, the formula above indeed are related to SHAP values.
Let's actually see the definition of Shapley values for the general case.

## How Shapley values are defined

To understand SHAP we have to go back to game theory and see how Shapley values are originally defined. 
...
The SHAP value of a feature value is then defined as its contribution to the payout, calculated as a weighted sum over all possible feature value combinations:
...
Here, S represents a subset of the features used in the model, j is the index of the feature for which we compute the SHAP value, $S$ a coalition of some features (without $j$), and $p$ is the number of features in the model.
...
For a game with combined payouts val+val^+^, the respective SHAP values are as follows:
...
There exists an alternative formulation of SHAP values in which the Dummy and Additivity axioms are replaced with a Linearity axiom; however, both formulations ultimately lead to the SHAP values.
...
Each feature gets assigned one SHAP value and the sum of SHAP values yields the prediction.
...
The SHAP interaction index, derived from game theory, is defined as:
...
We then average the values over all possible feature coalitions $S$, similar to the SHAP value computation.
...
The next chapter will delve deeper into the theoretical side of things, focusing on how to estimate SHAP values, because having just one method would be too simplistic.
