...
From the SHAP perspective, classification is similar to regression, except the scale the score or probability for a class.
...
- Using a subset of the training data as a background dataset, which can improve computation speed at the cost of less accurate SHAP value estimates. In the [Regression Chapter](#regression) we'll discuss more elaborate choices of background data in depth.
...
Another point of interest is that the model has two output2: the probability of earning less than \$50k and the probability of earning more than \$50k.
...
This plot shows the probability of this individual to earn less than \$50k.
We can see it's the figure from before except all SHAP values are multiplied by -1.
...
This turn the probabilities into log odds.
In mathematical terms, it's rearrangement of the terms in the logistic regression model:
...
Whenever rare events, anomalies, and extreme probabilites matter, go with logits.
