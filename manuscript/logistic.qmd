...
The novelties: 
We only use a subset of the training data as background dataset.
Also, we apply SHAP not only to the model, but the entire pipeline.
For a good reason.
...
- We use Shapley values applied to $\tilde{f}$ instead of $f$
...
Therefore we have to also indicate the class for which we want to access the Shapley values.
...
The interpretation is mostly the same as for regression, only that the outcome is on the probability level and that we have to decide for which class we want to interpret the Shapley values.

Let's examine the Shapley values for the alternative class.
...
![Probabilities versus Logits](images/logits.png)
