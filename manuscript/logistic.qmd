...
- Decide whether to interpret the model on the level of probability or log odds
...
I will skip the interpretation here, since we have already done it before and it's the same information, just presented differently.

Then we can also use the force plot:
...
Note that the force plot is with Javascript and you can hover over the plot to reveal more insights.
It also means that you can embed it, for example, in a Jupyter notebook, but not in a PDF file.
What you actually see above is a screenshot of a force plot.
Why is it called force plot?
Because the SHAP values are plotted as forces, here arrows, that can either push to increase the prediction or decrease it.
If you compare it to the waterfall plot, it's like a horizontal arrangement of the arrows.

Personally, I like the waterfall plot the most since it not only is easy enough to read, but also show the efficiency axiom, that the sum of all SHAP values is equal to the difference of f(x) and E[f(x)].
...
We can observe that Marital Status and Education are the two most important features (on the log odds scale).
...
You can also change the ordering.
...
I see this plot as an exploratory tool.

## The heatmap plot

Another type of plot is the heatmap plot.
It's another form of showing all individual SHAP values, but without aggregating them like in the dependence or importance plot.
The heatmap plot shows in each row one of the features and the SHAP values are plotted in the form of a heatmap.
The special thing is that on top there is a curve for the predicted value, so you can relate how predicted outcomes and SHAP values for certain features fit together. 
The ordering of the instances, the "columns" in this plot if you will, is based on clustering, same as in the multi-forceplot above.

```{python}
shap.plots.heatmap(sv_logit[:,:,0])
```
