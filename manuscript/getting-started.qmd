...
In this chapter, we'll explore the use of SHAP values to explain a prediction using the `shap` library.
...
We can compute the SHAP values by providing the following code:
...
When building prediction model, you separate training and testing data to avoid overfitting and getting a fair evaluation.
...
Separation avoids that you "replace" feature values for a data point with it's own value, because it both appears in background data and data points to be explained.
...
Let's visualize the SHAP values for data point with index 1 (second row in the DataFrame).
...
However, sometimes the water can flow in both directions, as SHAP values can be either negative or positive.
...
i = 1
y = model.predict_proba(X_test)[i][1]
...
```{python}
shap.waterfall_plot(shap_values[34])
```

Here we see an example where Relationship=0 has a negative effect, which is mostly cancelled through the Education feature.
In general it's a good example of how SHAP values can cancel each other out -- the actual prediction is close to the average prediction.
...
The plot displays SHAP values for all features and data points for which we computed the values.
This plot provides a comprehensive overview of several aspects:

- The importance of each feature, defined as the sum of absolute SHAP values.
...
This order is determined by the sum of absolute SHAP values for that particular feature across the data.

::: {.callout-warning}

Visual spread doesn't equal variance:
While feature Capital Gain has a huge spread, there is more variance of SHAP values for the Relationship feature.
...
- The largest SHAP values in both directions are for the Capital Gain feature, which indicates a significant variance.
- High values of Relationship were associated with (mostly) positive SHAP values and therefore a positive contribution towards the prediction. However, there is also a small cluster of data where a high Relationship had negative SHAP values.
- The higher the Age, the larger the SHAP values; this implies that Age is mostly positively correlated with making more than \$50k a year.
...
Let's ignore the encoding for training for now; however, it's also a problem for interpretation: we want to see the different categories and how the SHAP values look for each class.
In the [Classification Chapter](#classification), we will dive deeper into this issue.
