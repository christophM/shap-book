...
Let's visualize the attributions (i.e., the Shapley values) for data point with index 1 (second row in the DataFrame).
...
The sum of all Shapley values equals the difference between the prediction ({y}) and expected value ({base_value}).
...
In general it's a good example of how attributions can cancel each other out -- the actual prediction is close to the average prediction.
...
- The importance of each feature, defined as the sum of absolute Shapley values
...
While feature Capital Gain has a huge spread, there is more variance of Shapley values for the Relationship feature.
