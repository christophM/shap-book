...
We begin with games, coalitions, and payouts, transitioning to machine learning later in this chapter.

In this chapter, we will progressively define a fair payout in a coalition of players and ultimately arrive at Shapley values (spoiler alert).
...
Adding Charlie to Alice and Bob's ride increases the cost to \$51 since Bob lives somewhat further away. 
...
However, one could argue that adding Alice to an empty taxi is more informative than adding Alice to a taxi already occupied by Bob.
...
We use these weights to continue our journey towards a fair cost sharing.
...
We could have made different decisions regarding how we "fairly" allocate the costs to the passengers.
For instance, we could have weighted the marginal contributions differently.
We could have divided the costs by 3.
Alternatively, we could have used solutions that depend on the order of passengers:
...
We now have the formula, but where did it originate from?
Lloyd Shapley derived it [@shapley1953value], but it didn't just materialize out of nowhere.
...
For instance, if Bob doesn't need the luxury version of the taxi, his marginal contributions would be exactly the same as Alice's.
...
Assuming there's no extra cost for including Charlie in any ride, all of Charlie's marginal contributions would be \$0.
The dummy axiom states that when all marginal contributions are zero, the Shapley value should also be zero.
This rule seems reasonable, especially as Charlie doesn't have any money.
...
The additivity axiom suggests that they could first calculate the ice cream costs and then the taxi costs separately.
