...
- Describe theory and application of Shapley values
- Calculate Shapley values for simple games
- Understand the axioms of Shapley values, namely efficiency, symmetry, dummy, and additivity

:::

To understand SHAP, we first have to discuss the theory behind Shapley values from game theory.
That's why we start with games, coalitions and payouts and make the transition to machine learning only afterwards.


## Who's Gonna Pay For That Taxi?

Forget about machine learning -- for now and let's look at a concrete example that can be seen as a coalitional game: sharing a taxi ride.

Alice, Bob, and Charlie had dinner together and get a taxi to get home.
Total cost: \$51. 
...
To answer that question of what a fair distribution of the costs are, we first ask simpler questions:
...
Alice and Bob live together, but adding Bob to the ride bumps the cost to \$25, because he insists of getting a more spacious, more luxury taxi, which adds a flat \$10 to the ride costs.
...
The coalition $\emptyset$ is a coalition without any players in it.
...
Most of them we have already been used in the example above, but here is an overview to solidify them and as further reference.
...
The three of them would find such a method rather useless.
...
The symmetry axioms says that, in such a case, both should pay exactly the same in the end.
Sounds fair.
...
The dummy axiom says that in such a case where all marginal contributions are zero then the Shapley value should also be zero.
That makes a lot of sense, both because it seems like a fair rule, but also since Charlie has no money.
