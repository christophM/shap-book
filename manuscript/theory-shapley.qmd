...
However, one could argue that we learn more about how much Alice should pay when we add her to an empty taxi compared to adding her to a ride with Bob.
...
We take these permutations to form coalitions, for example for Alice.
Each permutation then maps to a coalition:
People that come before Alice in the order are in the coalition, people after are not.
Since in  coalition the order of passengers doesn't matter, some coalitions will occur more often than others when we iterate all permutations like this:
