...
Next, we replace `cat-banned` from the coalition with a random value of the cat allowed/banned feature from the same apartment that we sampled.
...
With each forwards plus backward generation for a permutation, we get marginal contributions for multiple features, not just a single one.
