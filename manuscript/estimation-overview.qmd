...
$$ \phi_j =  \sum_{S\subseteq\{1,\ldots,p\} \backslash \{j\}}\frac{|S|!\left(p-|S|-1\right)!}{p!}\sum_{k=1}^{n}\left(\hat{f}(x^{(i)}_{S \cup \{j\}}, x^{(k)}_{C \backslash \{j\}})- \hat{f}(x^{(i)}_S, x^{(k)}_C)\right)  $$
