...
$$ \phi^{(i)}_j =  \sum_{S\subseteq\{1,\ldots,p\} \backslash \{j\}}\frac{|S|!\left(p-|S|-1\right)!}{p!}\sum_{k=1}^{n}\left(f(x^{(i)}_{S \cup \{j\}}, x^{(k)}_{C \backslash \{j\}})- f(x^{(i)}_S, x^{(k)}_C)\right)  $$
...
If you are familiar with the original SHAP paper [@lundberg2017unified], then you might be familiar with the Kernel estimation method, because that's how SHAP was originally suggested to be estimated.
