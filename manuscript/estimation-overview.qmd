...
- The value function used by SHAP requires integration over the feature distribution. But sine we only have data and don't know the distributions, we have to use estimation techniques such as Monte Carlo integration.
- Machine learning models often have lots of features. Since the number of coalitions increases exponentially with $2^p$ it's often unfeasible to compute the marginal contributions of a feature to all coalitions, but instead we have to sample coalitions.

Let's first assume we only have a few features for which we can still iterate through all coalitions, so we can focus on the estimation of the SHAP values from data.
...
But we don't have these distributions, so instead we can use the Monte Carlo estimator.
...
Let's simulate what it would like like starting with drawing just 1 sample, which means we would have only 1 Monte Carlo sample which of course makes for a very unstable average, but it helps to get started with the concept.
...
This is how we estimate the value function for a coalition of features $S$:
...
And the marginal contribution of a feature $j$ added to a coalition $S$ becomes:

$$\ \hat{\Delta}_{S,j} = hat{v}(S \cup \{j\})  - \hat{v}(S) =  \frac{1}{n}\sum_{k=1}^n \left( f(x^{(i)}_{S \cup  \{j\}}, x^{(k)}_{C \backslash \{j\}}) - f(x^{(i)}_S, x^{(k)}_{C}) \right) $$
...
For your convenience, the marginal contribution again:

$$ \hat{\Delta}_{S,j} = \frac{1}{n}\sum_{k=1}^{n}\left(f(x^{(i)}_{S \cup \{j\}}, x^{(k)}_{C \backslash \{j\}})- f(x^{(i)}_S, x^{(k)}_C)\right)  $$
...
## Sample coalitions if there are too many

Going through all the coalitions isn't feasible when the number of features and therefore the number of coalitions gets too large.
That's when we have to rely on estimation techniques that don't require going through all coalitions.

There are two solutions:

- For some cases, we can make use of the structure of the model. For example for purely additive models, like linear regression models (without interaction terms) it's enough to compute 1 marginal contribution per feature. Also for other models such as neural networks there are so-called model-specific estimation methods that avoid a full iteration through the coalitions.
...
Note how with this version we not only get information about one of the features, but with forwards + backward pass we get two marginal contributions per features.
And by repeating the permutation sampling we get even better estimates.
The more often it is repeated, the more marginal contributions are estimated and the closer the final estimation of the SHAP values will be.

But how do we get from here to SHAP values?
Actually, it's with a really simple formula.
...
But you can also define it via permutations.
Let $m$ enumerate permutations of the features with $o(k)$ being the k-the distribution, then SHAP can be estimated with:

$$\hat{\phi}_j^{(i)} = \frac{1}{m} \sum_{k=1}^m \hat{\Delta}_{o(k), j}$$

I gotta explain $\hat{\Delta}_{o(k), j}$:
We have an permutation $o(k)$.
...
But what is $m$?
If we want to sum over all coalitions, it's $m = p!$.
...
If you view the equation above, the $m$ can actually also be just a sample of all possible permutations.
To get a rough idea of how many permutation: 10 is for example the default in the `shap` package.

But since we do forwards and backwards passes, the formula is actually:

$$\hat{phi}_j^{(i)} = \frac{1}{2m} \sum_{k=1}^m (\hat{\Delta}_{o(k), j} + \hat{\Delta}_{-o(k), j})$$

Where $-o(k)$ is the backwards version of the permutation.
...
One reason that we already talked about is that we get to reuse the resulting coalitions and save a bit of computation.

But this permutation procedure has an additional bonus: it ensures that the efficiency axiom is always satisfied, meaning when you add up the SHAP values they will exactly result in the prediction (minus the average prediction).
...
- If possible, the 'auto' option picks a model-specific version, for example the tree estimator when the model is a random forest. But only for the following cases: linear, additive, and tree-based.
- Else, if there are 10 or fewer features, the exact estimation is used.
