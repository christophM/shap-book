...
- Machine learning models often possess many features. As the number of coalitions increases exponentially with the number of features ($2^p$), it's often unfeasible to compute the marginal contributions of a feature to all coalitions. Instead, we have to sample coalitions.
...
Next, we replace `cat-banned` from the coalition with a random value of the cat allowed/banned feature from a randomly drawn apartment.
...
By estimating the SHAP values for all feature values, we can obtain the complete prediction distribution (minus the average) among the feature values. 
...
Estimation through permutation works by creating a random permutation of an instance's feature values and then performing a forward and backward pass of coalitions. 
...
This is the forward pass. Next, we iterate backwards:
...
With this method, we don't only gather information about a single feature, but with a forward and backward pass, we obtain two marginal contributions per feature.
By repeating the permutation sampling, we achieve more accurate estimates.
The more permutations we sample and iterate over, the more marginal contributions are estimated, bringing the final SHAP values closer to their exact value.
...
But SHAP value can also be defined via permutations.
...
But since we perform forward and backward passes, the formula looks like this:
...
The permutation procedure with forward and backward passes, also known as antithetic sampling, performs quite well compared to other SHAP value sampling estimators [@mitchell2022sampling].
...
The "Background Data" column indicates whether the estimation method requires data to be estimated.

| Estimation Method| Estimation Idea | Model-specific? | Background Data | Inspiration |
| --- | --------- | --- | --- | --- |
| Exact | Iterates through all background data and coalitions. | Agnostic | Yes | - |
...
| Additive | Simplifies SHAP estimation based on additive nature of the model | GAMs | Yes | - |
...
| Gradient | Computes the gradient of the output with respect to the inputs. | Gradient-based | Yes | Input Gradient |
| Deep | Backpropagates SHAP value through units of the neural network. | Neural Networks | Yes | DeepLIFT |
...
The original SHAP paper [@lundberg2017unified] introduced the Kernel method, which involves sampling coalitions and utilizing a weighted linear regression model to estimate SHAP values.
