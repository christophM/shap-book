...
In 1953, he published "A Value for n-Person Games"[@shapley1953], introducing Shapley values.
...
In fact, machine learning didn't really exist in any form that we would recognize now.
...
They proposed using Shapley values to explain predictions of individual predictions.
Four years later, in 2014, they published another paper with an improved method for computing Shapley values [@vstrumbelj2014explaining], where they essentially suggested the sampling estimator (see [Chapter Estimation](#estimation)).
...
- Explainable AI/Interpretable ML wasn't as widely recognized at the time
- The papers by \v{S}trumbelj and Kononenko didn't include code, making it harder to use
...
Let's examine the events that led to Shapley values becoming popular in ML.
...
I view this LIME paper as a catalyst for the field of explainable AI and interpretable ML.
That's a bit subjective because it served as a catalyst for me, your humble author Christoph.
...
The prevailing sentiment at the time was: Oh, we are developing increasingly advanced machine learning algorithms, such as deep neural networks.
But, gosh, look at them, we have no idea how these models generate their predictions; how can we trust them?
...
This is a major conference, and if your research is published there as a machine learning engineer, it's more likely to draw attention.
But what was the SHAP paper about?
After all, Shapley values for ML were already defined in 2010/2014.

Lundberg and Lee reformulated the estimation of Shapley values as a weighted linear regression using the [Kernel estimator](#estimation).
...
1. A new estimator for Shapley values
2. Unification of several existing methods
I'm not particularly fond of the second framing.
Although having a framework that connects different methods is useful, SHAP itself doesn't truly unify them, as you need to modify other methods to produce Shapley values.
But that's a separate discussion.
Regarding the new estimator, using the `shap` package today means Kernel SHAP is no longer in use, making the paper somewhat historical.
I know I may sound harsh, but in this chapter, I want to not only discuss Shapley values' milestones in ML but also reflect on their popularity and whether it's justified.
...
- Solid Python implementation

This last point shouldn't be underestimated, as it allowed people to incorporate Shapley values into their projects.

Since then, SHAP has continued to grow in popularity.
Another significant development occurred in 2020 when Lundberg et al.[@lundberg2020local] proposed an efficient computation method for SHAP specifically tailored to tree-based models.
This was a significant advancement, as tree-boosting performs well in many applications, making it possible to quickly estimate Shapley values for state-of-the-art models.

Lundberg's other notable achievement involved extending SHAP beyond individual predictions by stacking Shapley values, akin to assembling Legos, to create global model interpretations.
...
Since then, other researchers have built upon SHAP and Shapley values, suggesting extensions, some of which can be found in the [extensions chapter](#extensions).
