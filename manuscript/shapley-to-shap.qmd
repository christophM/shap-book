...
$\int\hat{f}(x_{1},\ldots,x_{p})d\mathbb{P}_{X_{C}}$ is just the prediction function $\hat{f}$, and for features in coalition S, we simply fill in the values $x^{(i)}_j$, and for the feature not in S, but in C, we integrate over those.
...
v(S \cup \{j\}) - v(S) &=& \int\hat{f}(x^{(i)}_S, X_C)d\mathbb{P}_{X_{C}} - E_X(\hat{f}(X))\\
                       ...
                       &=& \int\hat{f}(X|X_S = x^{(i)}_S)d\mathbb{P}_{X_{C}}- \int\hat{f}(X | X_{S \cup \{j\}} = x^{(i)}_{S \cup\{j\}})d\mathbb{P}_{X_{C \backslash \{j\}}} \\
\end{align}


## Piecing it all together

Putting all these terms together and putting them into the Shapley value equation, we get the following:

$$
\phi_j =  \sum_{S\subseteq\{1,\ldots,p\} \backslash \{j\}}\frac{|S|!\left(p-|S|-1\right)!}{p!}\left(\int\hat{f}(X|X_S = x^{(i)}_S)d\mathbb{P}_{X_{C}}- \int\hat{f}(X | X_{S \cup \{j\}} = x^{(i)}_{S \cup\{j\}})d\mathbb{P}_{X_{C \backslash \{j\}}} \right) \\
$$
