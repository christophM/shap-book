...
\begin{align*}
v(S \cup {j}) - v(S) &= \int f(x^{(i)}S \cup X_C) d\mathbb{P}{X_{C}} -\mathbb{E}(f(X)) \\
&\quad - \left(\int f(x^{(i)}{S \cup {j}} \cup X{C \backslash {j}})d\mathbb{P}{X{C \backslash {j}}} - \mathbb{E}(f(X))\right) \\
&= \int f(x^{(i)}{S \cup {j}} \cup X{C \backslash j}) d\mathbb{P}{X{C \backslash {j}}} \\
&\quad - \int f(x^{(i)}{S} \cup X_C) d\mathbb{P}{X_{C}}
\end{align*}
