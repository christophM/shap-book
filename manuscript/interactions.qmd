...
<!-- TODO: Reread chapter, also GPT it -->
...
Let's plot what the prediction looks like, since this gives us a an idea.
...
Alright, for this random forest we will now get us some Shapley values.
We know that, based on the axioms, Shapley values fairly attribute the prediction to the individual features.
For the simulated example it's clear that the first two terms
...
- The first term is fully attributed to $x_1$
- The second term is fully attributed to $x_2$
- The third term must be split between the two features

But let's have a look what the Shapley values look like:
...
- The further away from the stage ($x_2$) the larger smaller the Shapley values
- The taller the fan ($x_1$) the smaller the Shapley values

But there are also some breaks in the patters: There are small fans that receive a large Shapley value. This is due to the interaction (sitting on shoulders).
...
1. There is a large jump at $x_1=160$. Make sense because we know that above 160cm there is no chance that you will get on someones shoulders (at least in our simulated data).
...
- One line is for people that are >7 away from the stage ($x_2$). Here we see the large jump, which makes sense, since at >160cm, these fans have no chance of getting on someones shoulders.
- The other line is for values of $x_2$ below 7. It has a smaller jump. But why does it have a jump at all? People in this "cluster" don't get to sit on someones shoulders when below <160cm.

The reason why the interaction also "bleeds" into the cluster where we wouldn't expect it has to do with how Shapley values work.
...
1. Tom, 161cm tall standing right next to Mia with a distance of 4 to the stage
...
They have a rather similar predicted joy for the concert, with Mia having a slightly worse prediction -- makes sense given she is slightly smaller an neither of them qualify for shoulders.

Let's have a look at their Shapley values.
...
The Shapley values differ quite substantially.
...
- Mia has a positive Shapley value for the distance, which also makes sense because she stands rather close to the front.
- For Tom, the directions are similar, which makes sense since they have the same distance and are almost the same height.
- But Tom's Shapley values are more extreme.

So even though Tom is actually taller than Mia, his height has a more negative impact on the prediction.
Neither have a chance to get on someones shoulders since they are both already in the front.


Even though Mia is smaller, her height has less of a negative impact compared to Tom.
And we also can't explain it with the interaction since Mia is to close to the band that she doesn't profit from the shoulder bonus.

But the surprising answer is that Mia's Shapley value actually is influenced by the interaction, even though she isn't directly affected by it. And this is because of how Shapley values are computed.

When we compute the Shapley value for Mia's height, her height value is either added to an empty coalition ($\emptyset$) or to a coalition where her stage distance is already present ($\{x_2=2\}$).
...
And since the shoulder bonus increases the concert joy, the Shapley value for Mia's height is increased compared to Tom's.

So even though Mia's closeness to the stage means that she wouldn't profit from the shoulder bonus, the Shapley value for her height still reflects the existence of that bonus.

That's why I would refer to Shapley values as having a global component: The interactions also influence data points further away.

Also that makes Shapley values different from a "what-if" analysis.
The what-if analysis of the height is:
How would the prediction change if we change the height of Mia or Tom?
The answer would be that the effect would be quite similar for both Mia and Tom would we, for example, increase their height by 10cm.
But Shapley values don't work that way.
...
- Shapley values are not to be interpreted as a what-if analysis
- Feature interactions are split across the Shapley values
