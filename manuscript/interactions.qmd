...
This chapter goes deep into a simulated example so we can learn how feature interactions affect SHAP values.
...
Both individually add to the price: the larger the room, the more expensive and you pay extra for rooms from which you can see the ocean.
In addition, size and view interact: For small rooms, the sea view adds less to the price compared to large rooms, since the small rooms aren't inviting enough for staying a long time in them.

We have to distinguish interactions in reality and in the model.
Even if an interaction exists in your data, if you use a purely additive model, the interaction won't show in the model.

:::

## A function with interactions

You are at a concert and who doesn't love when you can actually see the band and not just the back of other people's heads.
It depends on your size and how close yo are to the stage how much you'll be able to actually see.
...
- Additional interaction: Fans that are small and far away from the stage get a "bonus": some nice person will allow them to sit on their shoulders and view the concert.

We simulate some this data and train a random forest.
...
Here is a visual of how the features height and distance relate to the prediction:
...
In the next step we produce explanations for the predictions.

## Computing SHAP values

Let's compute some SHAP values:
...
The dependence plot colors the points by values of feature $x_2$, since we provided the shap values also for the color option.
...
3. There are two "clusters" of points: one with a small jump and one with a large jump. The one with the smaller jump is a bit tougher to explain.
