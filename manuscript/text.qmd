...
Options 1 and 2 are more reasonable than option 3, as they can lead to grammatically incorrect sentences that might be atypical for the training distribution, whereas option 3 could introduce wildly different texts.
...
We'll use Hugging Face transformers, which make implementation incredibly easy:
...
But why? As humans, we know the keyword "scam" is the reason.
...
We see that "IBM" and "sc" contributed the most to the negative sentiment, especially "sc".
...
To demonstrate this, let's consider another example using Shapley values based on sentences.
...
I used a whitespace " " as mask token, which seems appropriate for dropping a sentence.
...
What happens here? The replacement acts as a reference. In one case, any sentence dropped from the coalition is replaced with "I love it," and in the other explanation, with "I hate it."

What changes? The base value shifts from strongly positive to negative. Every sentence is now interpreted in contrast with the replacement. This was also true before, but previously we replaced it with an empty string, which is more neutral than the sentences provided.

Avoid using extreme masking tokens, as they might not make sense. However, using more specific tokens can be beneficial.
The lesson here is that masking matters, as it serves as background data. Be mindful of the replacement and consider whether it makes sense or if alternatives should be tested.
...
The output is in the probability space between 0 and 1, which requires a logit transformation. Additive explanations, however, perform better on linear scales, such as logits, which occur just before the 0 to 1 squeezing.
...
The unique aspect of text-to-text models is that they have multiple outputs: Each token produced by the neural network is considered its own prediction.
You can think of it as a classification task: given some text input, classify which token should come next.
However, it's more than classification since we also obtain the output score.
...
Since the outputs are at the token level, which depends on the implementation of the neural network, that's the level used to generate explanations.

In essence, it's straightforward:
For text-to-text models, each output token is treated as an individual prediction, similar to multi-class classification, and we can compute Shapley values for each.

So, if the tokenized input has length $n$ and the tokenized output has length $m$, we get $n \cdot m$ Shapley values.
Remember that the tokenization level of input can be controlled by the user.
For the example above, if the user chooses word-level tokenization for the input, the first token of the output, e.g., "No", gets $n$ Shapley values; then, the next token "!" gets $n$ Shapley values, and so on.
...
The significant difference is that we receive not only the words but also the scores for the words, which are essential for computing Shapley values.
...
# Enable sampling instead of always returning the most likely character
...
TODO: It seems that the config setting does not change anything here.
Find a way to modify it.

```{python}
torch.manual_seed(0)
# Setting model to decoder is necessary to prevent input repetition.
...
TODO: fix plot above
This explanation corresponds to the first word.
TODO: interpretation
...
In this case, we will obtain an explanation for each output word.

Hugging Face Auto classes can automatically derive the relevant model for a specific task, or retrieve a model based on username and model-name.
...
CONTINUE HERE
Determine how to create a functional translation. Consider referring to the SHAP documentation tutorial.
