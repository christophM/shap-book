...
I will demonstrate both what SHAP values do and how background data sampling should appear when respecting the conditional distribution of the data.
```{python}
#| label: fig-sampling
...
The plot on the right illustrates that when we ignore the correlation between $X_1$ and $X_2$ and sample $X_2$ independently from $X_1$, we create new data points outside of the distribution.
This is what occurs with SHAP values.
...
This procedure is actually it's own game, called Owen's game and we get Owen's value back.
But it's tightly connect to Shapley values, as we will see in the following example.
...
The benefit is that for groups of correlated features, we obtain a single SHAP value for interpretation.
...
However, this doesn't actually violate the Dummy axiom, since based on the new payout using conditional distributions, they still qualify as SHAP values.
...
As mentioned above, the game changes and features not utilized by the model can have non-zero SHAP values.
