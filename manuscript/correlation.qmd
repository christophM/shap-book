...
- This implies that we are exploring model predictions in these areas.
...
Next, let's select a data point for which we will simulate drawing from the background data.
...
I will demonstrate both what Shapley values do in red and how background data sampling should appear when respecting the conditional distribution of the data.
```{python}
#| label: fig-sampling
...
It's difficult to gauge the severity of the problem, but inpainting is a form of conditional sampling, and blurring is even more unique as it doesn't use all the feature's information.
...
If this is the case, we can leverage a wider range of techniques to reduce the number of features used:
...
As hinted in @fig-sampling, it's also possible to use different sampling methods.
...
However, this doesn't violate the axiom, since based on the new payout using conditional distributions, they still qualify as Shapley values.
