...
If you want to employ conditional sampling in `shap`, use the Linear explainer with the `feature_perturbation='correlation_dependent'` option.
The Tree explainer provides a similar option called `feature_perturbation= 'tree_path_dependent'`, which uses the splits in the underlying tree-based model to ensure SHAP is not extrapolating.
