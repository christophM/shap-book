...
- Input: image data
- Output: score (can be multi-dimensional for multi-class classification)
...
From the parameters, we can see that it blurs the absent image parts.
Later, I'll show you how this actually looks like.
First, we'll compute Shapley values for two images: a cheeseburger and a pocket watch.
...
We'll choose the top 3 classes (`topk`) and 2 images.
...
The code above calculates all Shapley values.
...
Examining the Shapley values, the burger's top part mainly influenced the classification towards bottlecap.
...
A disadvantage of computing the Shapley values for multiple images here is that the color scale for the Shapley values, seen at the bottom, is for all images.
...
Recall the theory about how Shapley values work:
Shapley values are about how much features (or groups of features) contribute to coalitions of features.
...
- `blur(16, 16)`: Blurring relies on kernel size, which can be set by the user. Larger values involve distant pixels in the inpainting process, and blurring is faster than inpainting.
...
This resembles the Shapley values concept, where there are only two "players" made up of the lower and upper parts of the images.
...
The blur(16,16) kernel, for example, doesn't change the image significantly.
...
Even as a human, I can still recognize the lower part as a burger, meaning the features aren't significantly removed.
...
As we use the Partition explainer, more evaluations allow for finer partitions and thus more localized superpixels.
...
- We obtain more fine-grained superpixels, which relates to the Partition explainer. With only 10 evaluations, we get 4 leaves, necessitating a tree depth of 2, resulting in 2 + 2 * 2 = 6 Shapley values (TODO: check Partition explainer).
- The Shapley values decrease as the partitions and the number of pixels become smaller. It makes sense that masking fewer pixels has less influence on the prediction.
- Computation time scales linearly with the number of evaluations.

However, the Partition explainer is not our only option for image classifiers. We can also generate explanations at the pixel-level, which we will explore in the next chapter.
