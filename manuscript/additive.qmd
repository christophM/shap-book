...
In this step, we introduce the concept of non-linear relationships between a feature and the target, while excluding interactions between the features.

## Introducing the generalized additive model (GAM)

Generalized additive models (GAMs) are perfect for modeling additive models with non-linear base functions.
...
We'll utilize the `interpret` Python library for this. You can install it with:
...
This is promising, as it indicates that by using a GAM and allowing non-linear feature effects, we improved predictions and revealed some non-linear effects.
...
- For this particular wine, the most crucial features were alcohol and free sulfur dioxide, whereas, in the linear model, it was residual sugar and free sulfur dioxide.
...
We understand that the GAM is additive.
This implies that we can inspect each feature in isolation to understand its effect without considering interaction effects.
...
Like the linear case, in the additive case, SHAP values accurately track the feature effect and align with what we would predict.
...
The following figure displays the SHAP feature importance for the random forest model previously trained to predict cervical cancer.
...
While the feature importance plot is beneficial, it doesn't provide information beyond the importances.
