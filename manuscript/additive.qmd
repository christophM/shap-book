...
The explainer here is a Permutation explainer.
Why isn't the Additive explainer method used here?
Because in `shap` the additive estimation is only implemented for `interpret.glassbox.ExplainableBoostingClassifier`, but we are using the Regressor.
...
- The quality predicted by the GAM is lower with around 6.0 compared to 6.4. 
...
We know that the GAM is additive.
This means that to understand the effect a feature has, we can look at features in isolation and don't have to think about interaction effects.
Let's have a look at the SHAP dependence plot for alcohol:
```{python}
shap.plots.scatter(shap_values[:,"alcohol"], show=False)
```


In the case of alcohol, we see a positive relation between alcohol levels and the SHAP values.
The higher the alcohol content, the higher the SHAP contribution.
...
We can plot the SHAP values and add the alcohol curve directly extracted from the GAM.
