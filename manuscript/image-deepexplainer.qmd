...
Though the network receives individual pixels as input, we don't need to use the same granularity for explanations.
...
There are essentially two options to replace a value with images:

- Replace them with background data, that is, with pixels from a dataset of other images.
